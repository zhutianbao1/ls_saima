...
class NewRankWidget extends BaseController {
	...
	public function rank_xfb(){
		...
		parent::xfb($conuty_name);
		$this->display('NewRank/rank_xfb');
	}
		...
		if(empty($f)){
			if(!empty($my[0]['CON'])){
				$sql="SELECT * FROM
					...
					(SELECT * FROM rank_year_info where rpt_month=".$month."-1) b,
			...
			$sql="SELECT * FROM
			(SELECT b.*,a.oa FROM rank_follow a,
				(SELECT distinct a.*,b.同岗位排名-a.同岗位排名 上升,a.amount-c.amount 相差 FROM
				(SELECT * FROM rank_year_info where rpt_month=".$month." and 同岗位人数<>'团队类无计算') a,
				(SELECT * FROM rank_year_info where rpt_month=".$month."-1) b,
				(SELECT * FROM rank_year_info where rpt_month=".$month." and 同岗位排名=1) c 
	...
	public function rank_my_info($bill_id,$month){
		...
		$sql_qlm="SELECT * FROM rank_qlm where bill_id='".$bill_id."' and config_id=".$id[0]['ID']."";
