...
class Model {
     ...
     * @return mixed
     */
    public function orcAdd($tbName,array $data) {
        if(empty($tbName)){
           $this->error    = 'para tbName allow not null';
           return false;
        }else{
            $data = $this->_dataFormat($tbName,$data);
            if (!$data) return false;
 
            //$sql = 'INSERT INTO '.$tbName.' ('.implode(',', $fields).') VALUES ('.implode(',', $values).')';
     ...
     * @return mixed
     */
    public function orcUpdate($tbName,array $data) {
        if(empty($tbName)){
           $this->error    = 'para tbName allow not null';
           return false;
        }else{           
    ...
    */
    protected function _dataFormat($tbName,$data) {
        ...
        foreach ($data as $key=>$val) {      
            ...
            if (array_key_exists(strtolower($key),$table_column)) {               
                ...
                if (is_int($val)) {
                    ...
                    $val = $val;
                } elseif (strpos($val,'.nextval')>0) {
                    //序列不处理
                }elseif (strstr($val,'date.')) {
                    //日期处理
    ...
    * @return array
    */
    protected function _tbFields($tableName) {
